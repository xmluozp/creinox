import { FACELESS as CONST, LOADING } from "../_constants";
import { facelessService as service } from "../_services";
...
function get_dropdown(pagination, searchTerms = {}) {
  return dispatch => {
    ...
    return service.get_dropdown(pagination, searchTerms).then(
      response => {
        dispatch(loaded);
        dispatch(done(response, CONST.GETDROPDOWN));
      },
      error => {
...
function get_bySearch(pagination, searchTerms = {}) {
  return dispatch => {
    ...
    return service.get_bySearch(pagination, searchTerms).then(
      response => {
        dispatch(loaded);
        dispatch(done(response, CONST.GETALL_SUCCESS));
      },
      error => {
        dispatch(loadedFailure);
        dispatch(failure(error.toString()));
