...
// ============================================ customized
function get_treeNotesById(financialLedgerId) { // 取子类
    ...
    const requestOptions = {
      ...
      const queryString = h_queryString({perPage:-1, orderBy:"path", order:"ASC"}, {root_id: financialLedgerId}, TABLENAME);
      console.log("search service, get by root:", queryString);
      return fetch(`${getUrl(URL)}?${queryString}`, requestOptions).then(handleResponse);


    // 后台需要取：根据Id匹配path+","+id。否则全部返回 SELECT * FROM financialLedger WHERE id = xx, path like CONCAT(financialLedger.path,"," ,financialLedger.id)
    ...
    // const url = './dataset/financialLedgerdata.json'
    // console.log("get by financialLedger service:", financialLedgerId);

    // return fetch(`${url}?financialLedgerId=${financialLedgerId}`, requestOptions).then(handleResponse);
}