...
// ******************************************************************* page setting
import { financialtransactionActions as dataActions, financialaccountActions } from "_actions";
...
export const withTablePage = () => {
  ...
  const renderOrderForm = (content, row) => {
    if( content) {
      return `[  ${ enumsLabel.contractType[row["order_form_id.row"].contractType]}] ${row["order_form_id.row"].code}`
    } else {
      return "无"
    }    
  }
   ...
   * @param {} param0
   */
  const CurrentPage = ({ onDelete, onGetByIdAccount, dataByIdAccount, pageName, ...props }) => {

    const financialAccount_id = parseInt(_.get(props, "match.params.financialAccount_id")) || 0;
    const preConditions = {financialAccount_id }

    const EDITURL = "/financial/financialtransactions/" + financialAccount_id;
    const CREATEURL = "/financial/financialtransactions/" + financialAccount_id +"/add";
    ...
    const topButtons = [
      ...
      { label: "打印银行对账表", color: "secondary", onClick: ()=> {alert("hello")} }
    ];
    ...
    return (
      ...
      <ToolBar buttons={topButtons }/>
        ...
        {...props}
          ...
          tableTitle={`${(dataByIdAccount && dataByIdAccount.row && dataByIdAccount.row.name) || "[查找账户中...]"}转账记录`}
